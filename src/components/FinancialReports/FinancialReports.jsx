...
export default async function FinancialReports(){
    ...
    let financialItem = []
    docsData.map((i)=> {
        if (i.title.includes("Checking")){
            financialItem.push(<Item title={i.title} location={i.location} key={i.location}/>)
        }else{
    ...
    return(
        <div class="container my-5 sm:my-12 mx-auto px-4 md:px-12">
            ...
            <h2 class="text-center text-lg py-4 lg:text-5xl lg:py-10 font-bold whitespace-nowrap text-gray-500 dark:text-gray-400">Events Financial Details</h2>
            <div class="flex flex-wrap -mx-1 lg:-mx-4 justify-center">
                <div class="grid grid-cols-1 sm:grid-cols-4 gap-4 px-1 ">
                    {item}
                </div>
            </div>
            <h2 class="text-center text-lg py-4 lg:text-5xl lg:py-10 font-bold whitespace-nowrap text-gray-500 dark:text-gray-400">Bank Financial Details</h2>
            <div class="flex flex-wrap -mx-1 lg:-mx-4 justify-center">
                <div class="grid grid-cols-1 sm:grid-cols-4 gap-4 px-1 ">
                    {financialItem}
                </div>
