...
// Load PDFViewer ONLY on client side (disable SSR)
...
useEffect(() => {
    ...
    <div className="min-h-screen bg-gradient-to-b from-white to-gray-100 dark:from-gray-900 dark:to-black text-gray-800 dark:text-gray-100 px-4">
      ...
      <section className="relative h-[80vh] flex items-center justify-center text-center bg-cover bg-center" style={{ backgroundImage: "url('/images/community-building/banner.jpg')" }}>
        ...
        <div className="relative z-10 max-w-4xl text-white animate-fade-in-up px-6">
          <h1 className="text-4xl sm:text-5xl font-extrabold drop-shadow-lg mb-4">Our Future Community Center</h1>
          <p className="text-lg sm:text-xl font-medium mb-6">Together, we're building more than a structure — we're building our legacy.</p>
          <div className="flex flex-col sm:flex-row justify-center gap-4 mb-8">
            <button className="bg-yellow-400 text-black font-bold px-6 py-3 rounded-full shadow-xl hover:scale-105 transition">💖 Donate Now</button>
            <button className="bg-white text-pink-600 font-bold px-6 py-3 rounded-full shadow-xl hover:scale-105 transition">🏛️ Join the Vision</button>
          </div>
          <div className="flex items-center justify-center gap-3 text-red-500">
            <FaTools className="text-5xl text-red-600 animate-bounce" />
            <h2 className="text-2xl sm:text-3xl font-extrabold drop-shadow-md">This Page is Under Construction!</h2>
          </div>
        </div>
      </section>
  ...
  <p className="text-red-500 text-sm">No presentation selected or available.</p>
)}

          </div>
          <div className="mt-4 text-center">
            <a href={selectedPPT} download className="inline-block bg-blue-600 hover:bg-blue-700 text-white font-bold px-6 py-2 rounded-full shadow transition">
  ...
  <p className="text-red-500 text-sm">No meeting note selected or available.</p>
)}

          </div>
          <div className="mt-4 text-center">
            <a href={selectedNote} download className="inline-block bg-indigo-600 hover:bg-indigo-700 text-white font-bold px-6 py-2 rounded-full shadow transition">
